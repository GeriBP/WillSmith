...
public class playerMove : MonoBehaviour {
    ...
    public GameObject reconnectScreen, explosion, pickUpExp, pauseScreen;
	...
	void Update () {
        if ((Input.GetKeyUp(KeyCode.P)|| Input.GetKeyUp(KeyCode.Escape)) && !paused) //Pause
        {
            pauseScreen.SetActive(true);
            paused = true;
            Time.timeScale = 0.0f;
        }
        else if ((Input.GetKeyUp(KeyCode.P) || Input.GetKeyUp(KeyCode.Escape)) && paused) //UnPause
        {
            pauseScreen.SetActive(false);
            paused = false;
            Time.timeScale = 1.0f;
        }
        if (Input.GetKey(KeyCode.Escape) && paused) //Exit Game From Pause
        {
            Application.Quit();
        }
